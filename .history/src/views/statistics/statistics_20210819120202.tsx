...
const Statistics = () => {
    ...
    let getdata = () => {
        api.getdata({ date: time }).then((res: any) => {
            ...
            if (num === 0) {
                setpiechart(res.data.total_data.filter((item: any, index: number) => {
                    return item.pay_type === 1
                }).map((item1: any, index: number) => {
                    item1.name = item1.type_name
                    item1.value = item1.number
                }))
            }
            console.log(setpiechart)
        }).catch(err => {
            console.log(err)
        })
    }
        ...
        <div style={{ height: "100vh", background: "#F5F5F5" }}>
            ...
            <div className="back3f">
                ...
                <div className="flex-sb  mat10 padlr10 pad-t20">
                    ...
                    <div className="flex font14">
                        {arr.map((item: any, index: number) => {
                            return <div key={index} className={`${styles.dataExpense} ${num === index && index == 0 ? styles.dataExpense1 : ''} ${num === index && index == 1 ? styles.dataIncome : ''}`} onClick={() => { setnum(index) }}>{item}</div>
                        })}
                    </div>
                </div>
            </div>
