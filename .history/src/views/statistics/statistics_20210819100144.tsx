            ...
            <div className="flex-sb back3f mat10 padlr10 pad-t20">
                ...
                <div className="flex font14">
                    {arr.map((item: any, index: number) => {
                        return <div key={index} className={`${styles.zhichu} ${active === index && index == 0 ? styles.dataExpense1 : ''} ${active === index && index == 1 ? styles.dataIncome : ''}`} onClick={() => { setActive(index) }}>{item}</div>
                    })}
                    {/* <div className={`${styles.dataExpense}`}>支出</div>
                    <div className={`${styles.dataExpense}`}>收入</div> */}
                </div>
            </div>
            <DatePicker
                visible={show}
