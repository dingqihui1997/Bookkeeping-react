...
const Statistics = () => {
    ...
    let pie1 = (list: any) => {
        let pie = []
        pie = (list.filter((item: any, index: number) => {
            if (num === 0) {
                return item.pay_type === 1
            } else {
                return item.pay_type === 2
        ...
        <div style={{ height: "100vh", background: "#F5F5F5" }}>
            ...
            <div className="back3f">
                ...
                <div className="flex-sb  mat10 padlr10 pad-t20">
                    ...
                    <div className="flex font14">
                        {arr.map((item: any, index: number) => {
                            return <div key={index} className={`${styles.dataExpense} ${num === index && index == 0 ? styles.dataExpense1 : ''} ${num === index && index == 1 ? styles.dataIncome : ''}`} onClick={() => { setnum(index), pie1(list) }}>{item}</div>
                        })}
                    </div>
                </div>
            </div>
