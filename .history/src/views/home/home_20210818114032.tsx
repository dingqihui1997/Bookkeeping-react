...
const Home = () => {
    ...
    const [value1, setValue1] = useState(dayjs(new Date()).format(`YYYY-MM-DD`));//当前时间
    ...
    let [month, setMonth] = useState(String(Date.now()))//月份
    ...
    let choice = (time: any, id: any) => {
        console.log(time, id)
        setMonth(String(dayjs(time).valueOf()))
        setType(id)
    }
    // 点击消费类型
    ...
    // 获取账单列表
    let bill = () => {
        api.bill({ date: month, page: 1, page_size: 5, type_id: })
    }
    // 添加账单
    let addbill = () => {
        let pay = 1//账单类型
        ...
        let oldTime = String((new Date(value)).getTime());
    ...
    return (
        <div style={{ height: '100vh', background: '#F5F5F5' }}>
            <Hometop list={list} list2={list2} choice={choice}></Hometop>
