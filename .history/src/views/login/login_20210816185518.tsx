...
const Login = () => {
    ...
    let register = () => {//注册
        if (!user) {
            ...
            Toast.show('请输入验证码')
        } else if (yanzheng && arr.join('') !== yanzheng) {
            Toast.show('验证码错误')
        } else if (value === false) {
            Toast.show('请勾选协议')
        } else {
    ...
    let zhuce = () => {//注册账号请求
        api.register({ username: user, password: password }).then((res: any) => {
            console.log(res)
            if (res.code === 200) {
                Toast.show(res.msg)
                setActiveindex(0)
    ...
    let login = () => {//登录请求
        api.login({ username: user, password: password }).then((res: any) => {
            console.log(res)
            if (res.code === 200) {
    ...
    return (
        <div className={`${style.login}`}>
            ...
            {activeindex === 0 ? <div>
                ...
                <div className='mar-le20 marr20' onClick={goto}> <Button block theme="primary">
                    登录
                </Button></div>
