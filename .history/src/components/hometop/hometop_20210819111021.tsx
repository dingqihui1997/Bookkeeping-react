...
const Hometop = (props: props) => {
    let [value, setValue] = useState(dayjs(new Date()).format(`YYYY-MM`));//
    let [visible, setVisible] = useState<boolean>(false)
    let list = props.list//支出
    let list2 = props.list2//收入
    let [show, setShow] = useState(false)
    let [time, settime] = dayjs(new Date()).format(`YYYY-MM`)//当前时间
            ...
            >
                <div style={{ height: 500, background: "#eee" }}>
                    ...
                    <div style={{ paddingLeft: 20, paddingRight: 20, paddingTop: 10 }}>
                        <div className={`${styles.all11} ${id == 'all' ? styles.expend : ''}`} onClick={() => { expend('all', '全部类型'), setId('all'), props.choice(time, 'all') }}>全部类型</div>
                        <div className='mat10'>支出</div>
                        <div className='flex-w mat10'>
                            {list.map((item: any, index: number) => {
                                return (<div key={index} className={`${styles.oneitem} ${item.id == id ? styles.expend : ''}`} onClick={() => { expend(item.id, item.name), setId(item.id), props.choice(value, item.id) }}>{item.name}</div>)
                            })}
                        </div>
