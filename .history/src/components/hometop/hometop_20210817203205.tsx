...
const Hometop = () => {
    const [value, setValue] = dayjs(new Date()).format(`YYYY-MM`);//时间
        ...
        <div>
            <div className={`${styles.hometop} `}>
                ...
                <div className={`${styles.time} flex-end`}>
                    ...
                    <div className={`${styles.all}`} onClick={() => { setShow(true) }}>{ }<i className={`iconfont icon-jiantou9 ${styles.icon}`}></i></div>
                </div>
            </div>
            <Popup
                visible={visible}
