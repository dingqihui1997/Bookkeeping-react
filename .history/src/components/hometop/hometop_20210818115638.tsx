    ...
    return (
        <div>
            <div className={`${styles.hometop} `}>
                <div className="flex">
                    <div className='flex1 color3f'>总支出:<span className="font22 font-w7">{props.totalExpense}</span></div>
            ...
            >
                <div style={{ height: 500, background: "#eee" }}>
                    ...
                    <div style={{ paddingLeft: 20, paddingRight: 20, paddingTop: 10 }}>
                        <div className={`${styles.all11} ${id == 'all' ? styles.expend : ''}`} onClick={() => { expend('all'), props.choice(time, id) }}>全部类型</div>
                        <div className='mat10'>支出</div>
                        <div className='flex-w mat10'>
                            {list.map((item: any, index: number) => {
                                return (<div key={index} className={`${styles.oneitem} ${item.id == id ? styles.expend : ''}`} onClick={() => { expend(item.id), props.choice(time, id) }}>{item.name}</div>)
                            })}
                        </div>
