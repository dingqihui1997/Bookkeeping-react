                    ...
                    <div style={{ paddingLeft: 20, paddingRight: 20, paddingTop: 10 }}>
                        ...
                        <div className='flex-w mat10'>
                            {list.map((item: any, index: number) => {
                                return (<div key={index} className={`${styles.oneitem} ${item.id == id ? styles.expend : ''}`} onClick={() => { expend(item.id, item.name), setId(item.id), props.choice(time, item.id) }}>{item.name}</div>)
                            })}
                        </div>
