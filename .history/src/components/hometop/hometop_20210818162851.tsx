...
const Hometop = (props: props) => {
    ...
    let expend = (item: any, name: string) => {//支出 收入
        setId(item.id)
    }
    return (
        <div>
                    ...
                    <div style={{ paddingLeft: 20, paddingRight: 20, paddingTop: 10 }}>
                        ...
                        <div className='flex-w mat10'>
                            {list.map((item: any, index: number) => {
                                return (<div key={index} className={`${styles.oneitem} ${item.id == id ? styles.expend : ''}`} onClick={() => { expend(item), '', props.choice(time, id) }}>{item.name}</div>)
                            })}
                        </div>
