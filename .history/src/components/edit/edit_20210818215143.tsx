...
const edit = (props: props) => {
    ...
    let upkey = (key: any) => {//键盘事件
        if (key == 'delete') {
            ...
            console.log(oldTime)
            price ? update(name, oldTime) : Toast.show('请输入金额')
            console.log(name)
        } else if (key == 'close') {
            console.log('close')
        } else {
    ...
    let update = (typename: any, oldTime: any) => {
        let pay = 1//账单类型
        active ? pay = 2 : pay = 1
        api.update({ id: props.zdid, amount: Number(price), type_id: id, type_name: typename, date: oldTime, pay_type: pay, remark: content }).then((res: any) => {
            console.log(res)
            if (res.code === 200) {
                setVisible(false)
            }
                ...
                <div style={{ background: '#fff' }}>
                    ...
                    <div className="padlr10">
                        ...
                        {active === 0 ? <div className={`flex ${styles.boxicon}`}>
                            ...
                            {props.list2.map((item: any, index: number) => {
