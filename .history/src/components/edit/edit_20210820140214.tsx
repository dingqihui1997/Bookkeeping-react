...
const edit = (props: props) => {
    ...
    let [data, setData] = useState<any>(null)//金额
    let [price, setPrice] = useState('')
    let [active, setActive] = useState(1)//支出收入控制下标
    let [list, setList] = useState([])//支出
    let [list2, setList2] = useState([])//收入
    const [value1, setValue1] = useState(dayjs(new Date()).valueOf());//当前时间
    // const [value2, setValue2] = useState(dayjs(new Date()).format(`YYYY-MM-DD HH:mm:ss`));//当前时间
    ...
    let upkey = (key: any) => {//键盘事件
        if (key == 'delete') {
            ...
            price ? (props.data ? update(name, oldTime) : addbill(name, oldTime)) : Toast.show('请输入金额')
            // console.log(name)
        } else if (key == 'close') {
            console.log('close')
        } else {
    ...
    useEffect(() => {
        ...
        if (props.data) {
            setData(props.data)
            setActive(props.data.pay_type)
            setPrice(props.data.amount)
            setContent(props.data.remark)
            setValue1(props.data.date)
            if (props.data.pay_type == 1) {
                setExpend(props.data.type_id - 1)
            }
            else {
                let n1 = props.data.type_id - 10 - 1
                setIncome(n1)
            }
        }
    }, [props.data])
    ...
    let update = (typename: any, oldTime: any) => {
        ...
        api.update({ id: props.data.id, amount: Number(price), type_id: id, type_name: typename, date: oldTime, pay_type: pay, remark: content }).then((res: any) => {
            if (res.code === 200) {
                props.setvisible()
                props.success && props.success(res)
            }
        }).catch(err => {
            console.log(err)
        })
            ...
            >
                <div style={{ background: '#fff' }} >
                    ...
                    <div className="padlr10">
                        <div className="flex-sb">
                            <div className='flex'>
                                {arr.map((item: any, index: number) => {
                                    return <div key={index} className={`${styles.zhichu} ${active === index + 1 && index == 0 ? styles.zhichu1 : ''} ${active === index + 1 && index == 1 ? styles.shouru1 : ''}`} onClick={() => { setActive(index + 1) }}>{item}</div>
                                })}
                            </div>
                            <div className={`${styles.timeicon} flex-ja`} onClick={() => { setShow(true) }}>{dayjs(Number(value1)).format(`MM-DD`)}<i className={`iconfont icon-jiantou9 ${styles.icon}`}></i></div>
                        </div>
                        <div className={`${styles.moeny}`}>￥{price}</div>
                        {active === 1 ? <div className={`flex ${styles.boxicon}`}>
                        ...
                        {flag ? <div className={`${styles.addremarks}`} onClick={() => { setFlag(false) }}>{data && data.remark ? data.remark : '添加备注'}</div> : <div className={`${styles.autoinput}`}>
                            <Input
                                rows={3}
                                autoHeight
                                showLength
