...
const edit = (props: props) => {
    ...
    let [active, setActive] = useState((props.data ? props.data.pay_type : 1))//支出收入控制下标
                        ...
                        <div className="flex-sb">
                            <div className='flex'>
                                {arr.map((item: any, index: number) => {
                                    return <div key={index} className={`${styles.zhichu} ${active === index + 1 && index == 0 ? styles.zhichu1 : ''} ${active === index + 1 && index == 1 ? styles.shouru1 : ''}`} onClick={() => { setActive(index) }}>{item}</div>
                                })}
                            </div>
