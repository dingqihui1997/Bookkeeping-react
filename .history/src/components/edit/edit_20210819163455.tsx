...
const edit = (props: props) => {
    let [active, setActive] = useState(props.data ? (props.data.pay_type) : 0)//支出收入控制下标
    let [show, setShow] = useState(false)//时间弹框
    let [arr, setArr] = useState(['支出', '收入'])
    let [price, setPrice] = useState(props.data ? props.data.amount : '')//金额
    ...
    let [expend, setExpend] = useState(props.data ? (props.data.type_id) : 0)
    let [income, setIncome] = useState(props.data ? (props.data.type_id) : 0)//收入下标
                        ...
                        <div className="flex-sb">
                            <div className='flex'>
                                {arr.map((item: any, index: number) => {
                                    return <div key={index} className={`${styles.zhichu} ${active - 1 === index && index == 0 ? styles.zhichu1 : ''} ${active - 1 === index && index == 1 ? styles.shouru1 : ''}`} onClick={() => { setActive(index) }}>{item}</div>
                                })}
                            </div>
                        ...
                        {active === 0 ? <div className={`flex ${styles.boxicon}`}>
                            <div className={`flex nowap`}>
                                {list.map((item: any, index: number) => {
                                    return <div key={index} className={`${styles.iconexpend}`} onClick={() => { click(item, index), setName(item.name) }}>
                                        <div className={`flex-ja ${styles.iconitem} ${index == (expend == 0 ? 0 : expend - 1) ? styles.activeicon : ''}`}>
                            ...
                            {list2.map((item: any, index: number) => {
                                return <div key={index} className={`${styles.iconexpend}`} onClick={() => { setIncome(index), setId(item.id), setName(item.name) }}>
                                    <div className={`flex-ja ${styles.iconitem} ${index == (income == 0 ? 0 : income - list.length - 1) ? styles.activeicon1 : ''}`}>
