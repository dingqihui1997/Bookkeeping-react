...
export default {
    ...
    // 修改个性签名
    edit_userinfo({ signature, avatar }: { signature: string, avatar: string }) {
    ...
    // 修改用户密码
    modify_pass({ old_pass, new_pass, new_pass2 }: { old_pass: string, new_pass: string, new_pass2: string }) {
    ...
    update({ id, amount, type_id, type_name, date, pay_type, remark }: { id: number, amount: number, type_id: number, type_name: string, date: string, pay_type: number, remark: string }) {
        return http.post('/api/bill/add', { id, amount, type_id, type_name, date, pay_type, remark })
    },
